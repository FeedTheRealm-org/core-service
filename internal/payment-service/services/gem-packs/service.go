...
type GemPacksService interface {
	...
	CreateGemPack(name string, gems int, price float32) (*models.GemPack, error)

	// UpdatePack updates the details of an existing pack.
	UpdateGemPack(packageId uuid.UUID, name string, gems int, price float32) (*models.GemPack, error)
