Feature: User Sign-Up
  ...
  Scenario: AC-2a Show error when email field is empty
    ...
    Then the response should include an error message "You must provide an email address to create an account."
  ...
  Scenario: AC-2b Show error when password field is empty
    ...
    Then the response should include an error message "You must provide a password to create an account."
  ...
  Scenario: AC-3 Prevent account creation with an existing email
    ...
    Then the response should include an error message "The email address is already in use by another account."
